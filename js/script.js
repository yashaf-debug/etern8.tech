...
document.addEventListener('DOMContentLoaded', () => {
  // keep burger + language code as in PROMPT #1
  const form = document.getElementById('brief-form');
  if (form) {
    // remove accidental mailto anchors inside the form
    form.querySelectorAll('a[href^="mailto:"]').forEach(a => a.replaceWith(a.textContent || 'Send'));
    const API_URL = '/api/brief';
    const submitBtn = form.querySelector('button[type="submit"]');
    const notice = document.createElement('div');
    notice.style.marginTop='10px'; notice.style.fontSize='.95rem';
    form.appendChild(notice);
    form.addEventListener('submit', async (e) => {
      e.preventDefault();
      notice.textContent=''; if (submitBtn) submitBtn.disabled = true;
      ...
      try{
        ...
        console.error(err); notice.style.color='#c62828'; notice.textContent='Oops. Please try again or email hello@etern8.tech.';
