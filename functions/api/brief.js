...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (body.company) return new Response('OK', { status: 200 });
    ...
    // 1) Email через MailChannels (входит в Cloudflare)
    // Документация: https://developers.cloudflare.com/email-routing/email-workers/mailchannels/
    const mailPayload = {
...
Time: ${timestamp}`
    ...
    if (!mailRes.ok) {
      ...
      console.log('MailChannels error:', t);
    }

    // 2) Telegram (если заданы переменные)
      ...
      await fetch('https://api.notion.com/v1/pages', {
        ...
        body: JSON.stringify({
          ...
          properties: {
            ...
            Page: { url: pageUrl || 'https://etern8.tech' },
